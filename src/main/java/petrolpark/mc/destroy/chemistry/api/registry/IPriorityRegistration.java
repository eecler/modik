package petrolpark.mc.destroy.chemistry.api.registry;

/**
 * An object whose intended use is to collect {@link IRegisteredChemistryObject} to store in a {@link IChemistryRegistry}.
 * It allows multiple {@link petrolpark.mc.destroy.chemistry.api.event.IChemistryEvent IChemistryEvent} subscribers to attempt to register objects with the same {@code ID}, by picking the one registered with the highest priority.
 * @since Destroy 0.1.0
 * @author petrolpark

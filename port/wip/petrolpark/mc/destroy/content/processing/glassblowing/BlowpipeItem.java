...
public class BlowpipeItem extends BlockItem implements CustomArmPoseItem {
    ...
    public static FluidIngredientOld getFluidIngredient(CompoundTag tag) {
        if (!tag.contains("RequiredFluid", Tag.TAG_STRING)) return null;
        return FluidIngredientOld.deserialize(GsonHelper.parse(tag.getString("RequiredFluid"), true));  
    };

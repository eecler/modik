...
public class ReactionInBasinRecipe extends BasinRecipe {
    ...
    public static ReactionInBasinRecipe create(Collection<FluidStack> availableFluids, Collection<ItemStack> availableItems, BasinBlockEntity basin) {
        ...
        tryReact: if (canReact) {
            ...
            availableFluids.stream().map(FluidIngredientOld::fromFluidStack).forEach(builder::require);
    ...
    /**
     * The outcome of {@link petrolpark.mc.destroy.chemistry.legacy.LegacyReaction reacting} a {@link petrolpark.mc.destroy.chemistry.legacy.LegacyReaction Mixture} in a Basin.
     * @param ticks The number of ticks it took for the Mixture to reach equilibrium
     * @param reactionResults The {@link petrolpark.mc.destroy.chemistry.legacy.ReactionResult results} of Reacting this Mixture
